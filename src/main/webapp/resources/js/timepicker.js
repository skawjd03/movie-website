		...
		return this.each(function() {
			...
			function set_value(event, close) {
				...
				if (tim.length !== 0 && mini.length !== 0 && (!settings.show_meridian || meri.length !== 0)) {
					...
					var hour = null;
					if(meri == 'PM'){
						hour = parseInt(tim)+12;
					}else{
						hour = parseInt(tim);
					}
					var h = parseInt($('#timepickMin').html())/60;
			    	var m = parseInt($('#timepickMin').html())%60;
			    	var year = selectedDate.split('/')[0];
			    	var month = selectedDate.split('/')[1];
			    	var date = selectedDate.split('/')[2];
			    	uploadStartDate = new Date(year,parseInt(month)-1,date,hour,parseInt(mini),0);
			    	uploadFinishDate = new Date(year,parseInt(month)-1,date,(hour+h),(parseInt(mini)+m),0);
			    	if(uploadFinishDate.format('a/p')=='오후'){
			    		$('#finishTime').html(uploadFinishDate.format('hh:mm PM'));
			    	}else{
			    		$('#finishTime').html(uploadFinishDate.format('hh:mm AM'));
			    	}
				}
